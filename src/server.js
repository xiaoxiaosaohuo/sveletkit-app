...
function parseBundleTree(file, public_static_path) {
  ...
  for (const file in bundle_tree) {
    ...
    const file_public_path = urljoin(public_static_path, file);
    clientImports[file_public_path] = {
      css: [...new Set(css)],
      js: [...new Set(js)],
    };
...
function buildPage(component, props_json, clientImports, serverClientMap) {
  ...
  clientImports[component_path].css.forEach(
    (f) => (head += `\n\t<link rel='stylesheet' href='${f}'>`)
  );
  clientImports[component_path].js.forEach(
...
const { clientImports, serverClientMap } = parseBundleTree(
  "client-tree.json",
  PUBLIC_STATIC_PATH
);
...
if (DEV) {
  ...
  server.use(
    p,
    sirv(path.join(__dirname, "client"), {
      setHeaders: (res) => {
